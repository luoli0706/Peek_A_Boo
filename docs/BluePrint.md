...
**版本：** v3.0 MVP（C++ Server + ENET 架构）
**日期：** 2026-05-02
**引擎：** Unity 6.3 LTS（URP / FPS 模板）
**类型：** PVP 第一人称躲猫猫 / 派对游戏

> v3.0 架构变更：传输层从 WebSocket (TCP) 迁移到 **ENET (UDP + 可靠层)**，C++ 标准升至 **C++20**。ENET 提供游戏网络需要的可靠有序/不可靠分通道传输，避免 TCP 队头阻塞。FlatBuffers 序列化延后到 Phase 1。
...
| 服务端 | C++20 独立进程 (MinGW编译, Linux部署) |
| 通信协议 | **ENET (UDP + 可靠层)** — 可靠有序通道(状态同步) + 不可靠通道(位置快照) |
| 序列化 | 手写二进制 (memcpy + 固定结构体)，Phase 1 迁移到 FlatBuffers |
| 构建系统 | CMake 3.20+ (服务端) / Unity默认 (客户端) |
...
```
server/                          ← 服务端项目根目录
├── CMakeLists.txt               ← CMake 构建定义（链接 libenet）
├── config/
│   └── server.json              ← 运行时配置（端口、tick_rate、max_players）
├── deps/
│   └── enet/                    ← ENET C 库源码（已下载到此处）
│       ├── include/enet/        ← 头文件
│       ├── enet.c / host.c / peer.c / protocol.c ...
│       └── CMakeLists.txt       ← ENET 自带构建
├── src/
│   ├── main.cpp                 ← 入口：创建 ENET Host，进入事件循环
│   ├── server.h / .cpp          ← Server 类：生命周期管理
│   ├── room.h / .cpp            ← Room 类：单局游戏房间，管理玩家列表
│   ├── player.h / .cpp          ← Player 数据结构
│   ├── gamemode.h / .cpp        ← GameMode 状态机
│   ├── protocol.h / .cpp        ← 协议：消息类型定义 + 序列化/反序列化
│   ├── tick.h / .cpp            ← Tick 定时器（20Hz 主循环，std::chrono）
│   └── types.h                  ← 全局枚举和常量
├── test/
│   └── test_client.c            ← C 命令行测试客户端（直连 enet）
└── README.md                    ← 编译和运行说明
```
...
2. enet_initialize() → enet_host_create(9000, 7, 2, 0, 0)
   参数: port=9000, max_peers=7, channels=2 (ch0可靠 ch1不可靠)
3. 事件循环 (enet_host_service, 50ms超时 / 20Hz)：
   while (running) {
       enet_host_service(host, &event, 50);
       switch (event.type) {
           case ENET_EVENT_TYPE_CONNECT:     → 创建 Player，发 Welcome
...
### 6.1 Phase 0：服务端 Hello World + 客户端连接（2-3天）
...
```bash
# 1. 运行 Tools/msys2-x86_64-latest.exe 安装 MSYS2
# 2. 打开 MSYS2 UCRT64 终端，安装编译工具：
...
```
具体步骤：
1. CMakeLists.txt（deps/enet/ 已就绪）：
   cmake_minimum_required(VERSION 3.20)
   project(Peek-A-Boo-Server CXX)
   ...
   target_link_libraries(server PRIVATE ws2_32 winmm)  # Windows socket libs

2. 写最小 main.cpp：
   enet_initialize();
   ENetHost* host = enet_host_create(&address, 7, 2, 0, 0);
   // 参数: 7个peer, 2个channel (ch0可靠 ch1不可靠)
   // 事件循环：
   while (running) {
       while (enet_host_service(host, &event, 50) > 0) {
           switch (event.type) {
               case ENET_EVENT_TYPE_CONNECT:
                   ...
                   send_welcome(event.peer);
                   break;
               case ENET_EVENT_TYPE_RECEIVE:
                   printf("Received %d bytes, msg_type=%d\n",
...
3. 编写 protocol.cpp — Welcome 消息：
   void send_welcome(ENetPeer* peer) {
       uint8_t buf[3];
       buf[0] = 0x10;        // msg_type: Welcome
       buf[1] = assign_id(); // player_id
       buf[2] = assign_role(); // 0=Seeker, 1=Hider
       ENetPacket* pkt = enet_packet_create(buf, 3, ENET_PACKET_FLAG_RELIABLE);
       enet_peer_send(peer, 0, pkt); // channel 0 = 可靠
   }
...
5. 运行：./build/server.exe，应打印 "Server listening on port 9000"
```

#### 客户端（Unity C# + ENet-CSharp）

```
具体步骤：
1. Assets/Plugins/ENet-CSharp/ 已就绪（Phase 0 已 git clone）
   - 将 Source/Managed/ENet.cs 挂到 Unity 脚本编译链
   - 将 Source/Native/ 中的 enet.dll（预编译或待编译）放入 Assets/Plugins/

2. 构建 enet.dll（如果 ENet-CSharp 未提供预编译版本）：
   cd Assets/Plugins/ENet-CSharp/Source/Native
   cmake -B build -G "MinGW Makefiles" -DENET_SHARED=1
   cmake --build build
   # 将 build/enet.dll 复制到 Assets/Plugins/

3. 创建 NetworkManager.cs：
   public class NetworkManager : MonoBehaviour {
       void Start() {
           Library.Initialize();  // ENet-CSharp 初始化
           ENet.Event netEvent;
           host = new Host();
           host.Create();
           address.SetHost("127.0.0.1");
           address.SetPort(9000);
           peer = host.Connect(address, 2); // 2 channels
       }
           ...
           while (host.Service(0, out netEvent) > 0) {
               switch (netEvent.Type) {
                   ...
                   case EventType.Receive:
                       byte msgType = netEvent.Packet.GetData()[0];
                       Debug.Log($"Received msg_type=0x{msgType:X2}");
...
4. 移除 FPS 模板中的 NetworkManager/NGO/UnityTransport 组件

5. 打包双开测试：
   - 服务端运行 ./server.exe
   - Unity Editor 运行第一个客户端（Play Mode）
   - 再 Build 一个 Standalone 客户端或使用 ParrelSync 双开
```

**Phase 0 验收标准：**
- C++ 服务端在 Windows 本地编译运行，监听 UDP :9000
- 首个 Unity 客户端连接成功，收到 Welcome（msg_type=0x10）
- 第二个 Unity 客户端（双开）同时连接，服务端控制台打印两个连接
- 客户端断开时服务端打印 "Client disconnected"
- **关键验证：** 两个客户端的 player_id 不同，角色分配正确

**用到的知识：**
- C++：CMake 基本配置、ENET Host/Peer 概念、`enet_host_service` 事件循环、`enet_packet_create` + `enet_peer_send`
- C#：ENet-CSharp API（`Host.Create` / `Connect` / `Service`）、`BitConverter`

### 6.2 Phase 1：核心循环 — 1v1（3-5天）

**目标：** 两个 Unity 客户端完成一整局躲猫猫完整循环。

#### 服务端

```
具体步骤：
1. 实现 Player 数据结构 (player.h)：
   struct Player {
       uint8_t id;
       std::string name;
       uint8_t role;       // 0=Seeker, 1=Hider, 2=Spectator
       float pos_x, pos_y, pos_z;
       float rot_y;
       uint8_t state;      // 0=normal, 1=crouching, 2=caught
       bool ready;
   };

2. 实现 Room 类 (room.h/cpp)：
   - room_id 生成
   - players 列表（std::vector<Player>，最多7个）
   - AddPlayer / RemovePlayer
   - AssignRoles（随机选1个Seeker）

3. 实现 GameMode 状态机 (gamemode.h/cpp)：
   - 状态枚举 + 转换逻辑
   - 倒计时管理（timer变量 + Tick中递减）
   - OnEnter 回调（进入新状态时的初始化）

4. 实现输入处理：
   - 收到 PlayerInput → 按移动方向更新Player位置
   - 简单速度系统：Seeker稍快，Hider正常
   - 跳过碰撞检测（MVP），Phase 3再加

5. 实现 Tag 判定：
   - 收到 TagAttempt(seeker_id, target_id)
   - 计算两人距离：< 3米 → 成功
   - 广播 TagResult 给所有客户端
   - 检查是否全部被抓 → RoundEnd

6. 实现 20Hz Tick 循环：
   - 在主循环的 `enet_host_service` 超时中累积 deltaTime
   - 每50ms: 处理积累的输入 → 更新位置 → 广播 PlayerStates (ch1 不可靠)
```
...
具体步骤：
1. 分离本地移动和网络同步：
   - 本地：维持 FPS 模板的 PlayerController 移动响应
   - 每帧：收集输入 → 发送 PlayerInput 到服务端
   - 收到 PlayerStates：更新其他玩家的 Transform

2. 实现 GameState 响应：
   - 收到 GameStateChange：
     Hiding → 如果我是Seeker → 全屏黑遮罩
     Seeking → 移除遮罩，激活Tag功能
     RoundEnd → 显示结算UI
   
3. 实现 Tag 交互：
   - Seeker 准星对准某处 → 左键
   - 本地 Raycast：如果命中其他玩家的 Collider
   - 发送 TagAttempt(target_player_id)
   - 收到 TagResult → 播放音效/特效

4. 实现简单 UI：
   - 倒计时显示（从 GameStateChange 的 countdown_seconds）
   - 状态提示文字（"躲藏中..." "搜索中..."）
   - 连接状态指示器
```

**验收标准：**
- 两个客户端 → 服务端 → 躲藏→搜索→Tag→结算 → 重开
- 移动同步无可见抖动
- Tag 判定准确

**用到的知识：**
- C++：状态机模式、`std::chrono` 计时、结构体+vector管理
- C#：FPS PlayerController 结构理解、Raycast、UI Text/Canvas

### 6.3 Phase 2：完整人数 + 高亮 + 观战（2-3天）
...
### 7.4 MinGW + CMake 快速启动指南

```
# 1. 运行 Tools/msys2-x86_64-latest.exe 安装 MSYS2（已完成）
...
| 决策 | 结论 | 原因 |
|------|------|------|
| 服务端语言 | C++ | 独立进程、Linux部署、专业架构 |
| 通信协议 v2→v3 | WebSocket(TCP) → **ENET(UDP)** | Party Mode共识：TCP队头阻塞对游戏致命，ENET可靠/不可靠双通道是业界最佳实践 |
| C++标准 | C++20 | GCC 14+完全支持，`std::span`/`std::format`/designated initializers 实用 |
| 序列化 | 手写二进制 → Phase 1 FlatBuffers | 零依赖起步，Phase 1 引入 schema 自动生成双端代码 |
| 网络库 | **libenet** | 超轻量(3源文件)、15年+生产验证、UDP NAT穿透友好 |
| 构建 | CMake + MinGW | 跨平台标准 |
...
- libenet 官方：http://enet.bespin.org/
- ENET GitHub：https://github.com/lsalzman/enet
- ENet-CSharp (Unity)：https://github.com/nxrighthere/ENet-CSharp
- MSYS2 安装：https://www.msys2.org/

---

**先用 C++ 把服务端跑起来。Phase 0 唯一的目标：两个 Unity 客户端实例通过 ENET UDP 连上同一个 C++ 服务端，服务器打印 "Player 0 connected" 和 "Player 1 connected"。**
